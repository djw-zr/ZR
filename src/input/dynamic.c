...
int             i       ;
TrkSectionNode  *tsnode ;

/*
 *  Generate initialise track section node
 */
      tsnode = (TrkSectionNode *)malloc(sizeof(TrkSectionNode));
      witem->u.dyn_track_obj.tsnode          = tsnode ;
      for(i=0;i<3;i++){
       witem->u.dyn_track_obj.gl_display_list[i] =   0 ;
       witem->u.dyn_track_obj.dist_level[i]      = 0.0 ;
      }
      init_track_section(tsnode) ;

      tsnode->type_of_node     = VECTOR_SECTION ;
      tsnode->length_of_vector = 6              ;
      tsnode->vector = (TrkVectorNode *)malloc(6*sizeof(TrkVectorNode)) ;
